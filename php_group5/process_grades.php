...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['student'], $_POST['written_work'], $_POST['performance_task'], $_POST['quarterly_assessment'], $_POST['subject'])) {
    ...
    // Prepare and execute statement to check if the grade entry already exists
    $sqlCheckGrade = "SELECT id FROM student_grades WHERE student_id = ? AND subject_id = ? AND total_grade IS NOT NULL";
    $stmtCheckGrade = $conn->prepare($sqlCheckGrade);
    $stmtCheckGrade->bind_param('ii', $studentId, $subjectId);
    $stmtCheckGrade->execute();
    $resultCheckGrade = $stmtCheckGrade->get_result();

    if ($resultCheckGrade->num_rows > 0) {
        // Grade entry exists, update the existing entry
        $sqlUpdateGrade = "UPDATE student_grades SET written_work = ?, performance_task = ?, quarterly_assessment = ?, total_grade = ? WHERE student_id = ? AND subject_id = ?";
        $stmtUpdateGrade = $conn->prepare($sqlUpdateGrade);
        $stmtUpdateGrade->bind_param('dddiid', $writtenWork, $performanceTask, $quarterlyAssessment, $totalGrade, $studentId, $subjectId);

        if ($stmtUpdateGrade->execute()) {
            $message = "Grade updated successfully. Total grade: $totalGrade";
        } else {
            $error = "Error updating grade: " . $conn->error;
        }
    } else {
        // Grade entry does not exist, insert a new entry
        ...
        if ($stmtInsertGrades) {
            $stmtInsertGrades->bind_param('iddddd', $studentId, $writtenWork, $performanceTask, $quarterlyAssessment, $totalGrade, $subjectId);

            if ($stmtInsertGrades->execute()) {
                $message = "Grade added successfully. Total grade: $totalGrade";
            } else {
                $error = "Error executing prepared statement: " . $stmtInsertGrades->error;
            }
        } else {
